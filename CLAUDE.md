...
**myseki (내자리 / マイセキ)** — 지하철 좌석 양도 모바일 웹앱. 같은 열차에 탑승한 승객들이 인상착의를 등록하고, 하차 예정 정보를 공유하여 빈 좌석을 찾을 수 있는 서비스.
...
- 모든 페이지에서 동일하게 적용
- KR/JP 토글은 **UI 언어만 변경** — 지하철 데이터(국가)와 독립적
...
### 입장 플로우
1. `Landing` → 시작
2. `CountrySelect` → 한국 🇰🇷 / 일본 🇯🇵 지하철 선택 (국가 선택 시 언어도 자동 세팅)
3. `LineSelect` → 호선 선택 (한국: 1~9호선, 일본: 야마노테선 등 9개 노선)
4. `Home` → 호차 번호 선택 (GPS 자동 감지 가정)
5. `AvatarDecorator` → 인상착의 등록 (직접 꾸미기 / AI 문장 생성)
6. `RegisterSeat` → 하차역 선택, 좌석 등록 완료
7. `SeatFinder` → 해당 호차 페이지 진입 (메인 화면)
...
- 하차가 가까운 순으로 상단에 정렬
- 각 승객의 아바타, 인상착의 확인 → **아바타를 보고 실제 사람을 매칭하는 것이 서비스 핵심**
- **대기 기능**: "대기" 버튼 클릭 → "대기중"(빨간색)으로 토글, 대기 인원 수 표시 (별도 페이지 없음, SeatFinder 내 인라인)

## Architecture

### Routing & State
`App.tsx`가 모든 페이지 상태를 관리. `currentPage` state로 페이지를 조건부 렌더링.

**글로벌 state (App.tsx에서 관리):**
- `currentPage` — 현재 페이지 (`Page` union type)
- `country` — 선택된 국가 (`'kr' | 'jp'`), 지하철 데이터 결정
...
- `SUBWAY_LINES` — 한국 1~9호선 역 목록
- `JAPAN_SUBWAY_LINES` — 일본 9개 노선 역 목록
- `getSubwayLines(country)` — 국가에 따른 노선 데이터 반환
...
### i18n
`src/i18n.tsx` — React Context 기반 다국어 시스템
- `I18nProvider` + `useI18n()` 훅 → `{ lang, setLang, t }` 반환
...
- 모바일 퍼스트 (max-width 430px, max-height 932px)
...
3. 아이콘 스타일을 전체적으로 덜 애플스럽게 (플랫/미니멀 방향)
